...
namespace ntdll
{
    ...
    template<typename _Ptr = PVOID, typename _Int = SIZE_T>
    struct $PEB
    {
        enum : UINT32
        {
            ...
            GdiHandleBufferSize     = constexpr(sizeof(_Ptr) == sizeof(UINT32) ? GdiHandleBufferSize32 : GdiHandleBufferSize64),
        ...
        _Ptr    Mutant;
        _Ptr    ImageBaseAddress;
        _Ptr    Ldr; // PEB_LDR_DATA
        _Ptr    ProcessParameters;
        _Ptr    SubSystemData;
        _Ptr    ProcessHeap;
        _Ptr    FastPebLock;
        _Ptr    AtlThunkSListPtr;
        _Ptr    IFEOKey;
        union
        {
        ...
        union
        {
            _Ptr    KernelCallbackTable;
            _Ptr    UserSharedInfoPtr;
        };
        ...
        _Ptr    ReadOnlySharedMemoryBase;
        _Ptr    HotpatchInformation;
        _Ptr    ReadOnlyStaticServerData;
        _Ptr    AnsiCodePageData;
        _Ptr    OemCodePageData;
        _Ptr    UnicodeCaseTableData;
        UINT32  NumberOfProcessors;
        UINT32  NtGlobalFlag;
        LARGE_INTEGER CriticalSectionTimeout;
        _Int    HeapSegmentReserve;
        _Int    HeapSegmentCommit;
        _Int    HeapDeCommitTotalFreeThreshold;
        _Int    HeapDeCommitFreeBlockThreshold;
        UINT32  NumberOfHeaps;
        UINT32  MaximumNumberOfHeaps;
        _Ptr    ProcessHeaps;
        _Ptr    GdiSharedHandleTable;
        _Ptr    ProcessStarterHelper;
        ...
        _Int    ActiveProcessAffinityMask;
        UINT32  GdiHandleBuffer[GdiHandleBufferSize];
        _Ptr    PostProcessInitRoutine;
        _Ptr    TlsExpansionBitmap;
        ...
        _Ptr    pShimData;
        _Ptr    AppCompatInfo;
        $UNICODE_STRING<_Ptr> CSDVersion;
        _Ptr    ActivationContextData;
        _Ptr    ProcessAssemblyStorageMap;
        _Ptr    SystemDefaultActivationContextData;
        _Ptr    SystemAssemblyStorageMap;
        _Int    MinimumStackCommit;
        _Ptr    FlsCallback;
        $LIST_ENTRY<_Ptr>  FlsListHead;
        _Ptr    FlsBitmap;
        UINT32  FlsBitmapBits[FlsMaximumAvailable / (sizeof(UINT32) * 8)];
        UINT32  FlsHighIndex;
        _Ptr    WerRegistrationData;
        _Ptr    WerShipAssertPtr;
        _Ptr    ContextData;
        _Ptr    ImageHeaderHash;
        union
        {
            UINT32      TracingFlags;
            struct
        ...
        _Ptr    TppWorkerpListLock;
        $LIST_ENTRY<_Ptr> TppWorkerpList;
        _Ptr    WaitOnAddressHashTable[128];
        _Ptr    TelemetryCoverageHeader;    // REDSTONE3
    ...
    template<typename _Ptr = PVOID>
    struct $PROCESS_BASIC_INFORMATION {
        _Ptr   Reserved1;
        _Ptr   PebBaseAddress;
        _Ptr   Reserved2[2];
        _Ptr   UniqueProcessId;
        _Ptr   Reserved3;
    };
